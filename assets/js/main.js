...
async function decryptSupabaseConfig() {
    try {
        ...
        if (!text) throw new Error('Decryption failed: Empty result');
...
const state = {
    ...
    searchCache:   new Map(),
    searchHistory: [],
    // FIX 2: Restore randomCount from localStorage so it survives page reloads
    ...
    async loadUserData(username) {
        try {
            ...
            if (response.status === 404) {
                console.warn(`No user file for ${username}`);
                return null;
            }
            if (!response.ok) throw new Error(`HTTP ${response.status}`);
...
const search = {
    // Tracks active RAF and setTimeout handles so they can be cancelled cleanly
    // FIX 1: Added for interrupt-based animation instead of silent drop
    _activeFrameId: null,
    _activeTimerId: null,
    ...
    json(query) {
        ...
        if (query.includes(':')) {
            ...
            // Comma/space separated: "买入 20" or "20,买入"
            ...
            // FIX 7: Added (\.\d+)? to support decimal prices like "买入10.5" or "10.5买入"
            /^[\u4e00-\u9fa5a-zA-Z]+\d+(\.\d+)?$/.test(query) ||
        ...
        const matches = state.workbookData.filter(row => {
            ...
            return Object.entries(conditions).every(([key, value]) => {
                ...
                if (key === '收盘价') {
                    // Allow ±0.5 tolerance for floating-point price matching
    ...
    corpus(query) {
        ...
        const answer  = this.generateResponse(this.detectIntent(query), best);

        // FIX 3: Do not cache fallback/error strings — they may resolve correctly next time
    ...
    // FIX 1: Cancel any in-flight RAF or setTimeout from a prior typeLines call
    _clearTypeLoop() {
        if (this._activeFrameId !== null) {
            cancelAnimationFrame(this._activeFrameId);
            this._activeFrameId = null;
        }
        if (this._activeTimerId !== null) {
            clearTimeout(this._activeTimerId);
            this._activeTimerId = null;
        }
    },

    // FIX 1: Interrupt an active animation and start the new one instead of silently dropping it
    typeLines(lines, element) {
        ...
        if (state.isAnimating) {
            ...
            this._clearTypeLoop();
            // Wait one frame for the cancelled RAF to observe the flag before starting fresh
        ...
        const typeNextLine = () => {
            ...
            const typeChar = timestamp => {
                ...
                if (charIndex < content.length) {
                    this._activeFrameId = requestAnimationFrame(typeChar);
                } else {
                    lineIndex++;
    ...
    // FIX 6: Listeners are set once via delegation in DOMContentLoaded — not re-added here
    updateHistory() {
...
const PeekXAuth = {
    ...
    async register(event) {
        ...
        try {
            ...
            // Local JSON backup — non-blocking
    ...
    async search() {
        ...
        // FIX 7: Added (\.\d+)? groups so decimal prices like "买入10.5" route correctly
        const isJSONQuery =
            query.includes(':') ||
    ...
    // FIX 5: Use module-level supabaseClient, not this.supabaseClient (which is always undefined)
    logout() {
        if (supabaseClient) {
    ...
    // FIX 4: Single postLogin() definition — the empty stub that was shadowing this is removed
    _postLogin() {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // FIX 6: Single delegated listener for history clicks — replaces per-item listeners
    // that were re-added on every updateHistory() call, causing a memory/event leak
